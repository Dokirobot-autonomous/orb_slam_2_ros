...
*/
    ...
    std::pair<cv::Mat,cv::Mat> Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp)
    {
        mImGray = imRectLeft;
        cv::Mat imGrayRight = imRectRight;
        ...
        return std::pair<cv::Mat,cv::Mat>(mCurrentFrame.mTcw.clone(),mCurrentFrame.covariance.clone());
    }
